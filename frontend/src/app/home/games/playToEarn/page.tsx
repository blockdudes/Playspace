...
export default function GamingHub() {
  ...
  const filteredGames = games.games?.filter((game: Game) =>
    ...
    game.gameType === "EARNING" &&
    (selectedCategory === "All" || game.gameCategory === selectedCategory )
  )
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {isLoading
            ...
            : filteredGames?.map((game: Game) => (
                <Card key={game.gameId} className="bg-gray-800/50 border-gray-700 overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/20 hover:-translate-y-1 group">
                  <div className="relative">
                    ...
                    />
                    <Badge className="absolute top-2 right-2 bg-purple-600">{game.gameCategory}</Badge>
                  </div>
                  <CardContent className="p-3">
                    <h3 className="font-semibold text-sm mb-1 text-white truncate">{game.gameName}</h3>
                    <div className="flex items-center text-xs text-gray-400 mb-1">
                      <Users className="h-3 w-3 mr-1" />
                      <span>{game.gamePlayerCount?.toLocaleString()}</span>
                      <Coins className="h-3 w-3 ml-2 mr-1" />
                      <span>{game.gameTokenSymbol}</span>
                    </div>
                    <div className="flex items-center">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`h-3 w-3 ${i < Math.floor(Number(game.gameRating)) ? 'text-yellow-400' : 'text-gray-600'}`} />
                      ))}
                      <span className="ml-1 text-xs text-gray-400">{Number(game.gameRating).toFixed(1)}</span>
                    </div>
                  </CardContent>
                  <CardFooter className="p-3 pt-0">
                    <Dialog>
                      ...
                      <DialogContent className="bg-gray-800 text-white max-w-3xl">
                        <DialogHeader>
                          <DialogTitle className="text-2xl font-bold">{game.gameName}</DialogTitle>
                        </DialogHeader>
                        <div className="flex flex-col md:flex-row gap-4">
                          <div>
                            ...
                            />
                            <p className="mt-4 text-sm text-gray-300">{game.gameDescription}</p>
                          </div>
                          <div className="space-y-4">
                            <div className="flex items-center">
                              <Users className="h-5 w-5 mr-2 text-purple-400" />
                              <span>{game.gamePlayerCount?.toLocaleString()} active players</span>
                            </div>
                            <div className="flex items-center">
                              <Coins className="h-5 w-5 mr-2 text-yellow-400" />
                              <span>Token: {game.gameTokenSymbol}</span>
                            </div>
                            <div className="flex items-center">
                              <Star className="h-5 w-5 mr-2 text-yellow-400" />
                              <span>Rating: {Number(game.gameRating).toFixed(1)}</span>
                              <div className="ml-2">
                                {[...Array(5)].map((_, i) => (
                                  <Star key={i} className={`inline-block h-4 w-4 ${i < Math.floor(Number(game.gameRating)) ? 'text-yellow-400' : 'text-gray-600'}`} />
                                ))}
                              </div>
                            </div>
                            <Button 
